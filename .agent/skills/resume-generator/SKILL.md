---
name: 运维简历生成与模拟面试
description: 基于运维技能等级白皮书，生成符合STAR法则的简历项目描述，并进行深度模拟面试演练
---
...
| 资源 | 路径 | 说明 |
|-----|------|------|
| 项目模板 | `Example.md` | 10个运维项目模板，包含演进等级、技术栈、量化指标 |
| 能力模型 | `Project-Skills/*.md` | 44个能力等级文件（1-5级定义） |
| 示例输出 | `Example/*.md` | 简历和面试问答的参考示例 |
...
3. **项目选择**：参考 Example.md 中的10个项目模板，让用户选择2-4个与其经验最匹配的项目

### 步骤2：阅读相关能力模型

根据用户选择的项目，阅读 `Project-Skills/` 目录下对应的能力模型文件，了解各等级的能力定义。
...
指标选择需结合用户提供的实际项目规模和 Example.md 中的项目成效数据进行调整。

### 输出存储

将生成的简历保存到 `Example/` 目录，命名格式：
...
5. **文件格式**：Markdown
6. **真实性**：量化数据必须真实可验证，可参考 Example.md 中的项目成效数据
